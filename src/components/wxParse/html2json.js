 ...
 */
  ...
  function html2json(html,bindData, windowWidth) {
    __wxImageArray = [];
    ...
    HTMLParser(html, {
      start: function(tag, attrs, unary) {
        ...
        if(node.tag == 'img'){
          ...
          node.attr.width = windowWidth;
          node.attr.height = windowWidth * node.attr['data-ratio'];
          node.from = bindData;
          if (unary) {
        ...
        if (bufArray.length === 0) {
          if(node.classStr && node.classStr.search('com-insert-embed') != -1){
            node = {};
          }
